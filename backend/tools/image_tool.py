...
def build_image_tool(provider: str, emit_image_generated: Callable) -> Callable:
    async def generate_image(prompt: str, style: str = "realistic") -> str:
        ...
        """
        logger.info("generate_image tool called — provider=%s prompt=%s style=%s", provider, prompt[:80], style)
        ...
        try:
            ...
            logger.error("generate_image tool failed — provider=%s error=%s", provider, exc)
